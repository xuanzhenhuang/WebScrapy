...
# 定义要抓取的网页列表
detail_links_all = ["https://www.ikea.com/de/de/cat/kunstpflanzen--blumen-20492/",
                    "https://www.ikea.com/de/de/cat/kunstpflanzen--blumen-20492/?page=2",
                    "https://www.ikea.com/de/de/cat/kunstpflanzen--blumen-20492/?page=3",
                    "https://www.ikea.com/de/de/cat/kunstpflanzen--blumen-20492/?page=4",
                    "https://www.ikea.com/de/de/cat/kunstpflanzen--blumen-20492/?page=5"
                    ]
...
image_save_directory = os.path.join(current_dir, "images_IKEA_DE")
data_file_path = os.path.join(current_dir, "productInfo_IKEA_DE.xlsx")
...
def fetch_data(url, image_save_dir):
    try:
        ...
        tab.get(url)
        product_info_list = []
        produck_links = tab.eles('.plp-fragment-wrapper')
        print(len(produck_links))
        for product in produck_links:
            try:
                # 提取产品货号
                ...
                if len(current_retailText) == 1:
                    ...
                    # 获取到价格
                    reg_retail = Result[0].split(delimiter2)[2].split(delimiter3)[0]
                    print(current_retail, reg_retail)

                # 获取产品平均得分
                review_info = product.ele('.plp-ratings plp-ratings--small plp-ratings--product-card notranslate')
                if review_info:
                    delimiter1 = "Bewertungen: "
                    delimiter2 = " von 5 Sternen. Bewertungen insgesamt: "
...
print("数据已成功保存到 productInfo_IKEA_DE.xlsx 文件中。")
